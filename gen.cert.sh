...
then
    echo
    echo 'Issue a wildcard SSL certificate with Liancheng ROOT CA'
    echo
...
openssl req -new -out "${DIR}/$1.csr.pem" \
    ...
    -config <(cat ca.cnf \
        <(printf "[SAN]\nsubjectAltName=${SAN}")) \
    -subj "/C=CN/ST=Jiangsu/L=Nanjing/O=Nanjing Liancheng Information Technology Group Co., Ltd/OU=$1/CN=*.$1"
